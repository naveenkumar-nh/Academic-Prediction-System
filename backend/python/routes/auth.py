...
def forgot_password():
    """Handle password reset requests from login page modal."""
    role = request.form.get('role')
    identifier = request.form.get('identifier', '').strip()

    if not role or not identifier:
        flash('Please select your role and enter your identifier.', 'error')
    ...
    if role == 'student':
        ...
        staff_id = student.created_by
        if not staff_id:
            # Fallback to the first admin if no attached staff
            admin = User.query.filter_by(role='admin').first()
            if admin:
                staff_id = admin.id
                recv_type = 'admin'
            else:
                flash('System error: No staff/admin found to receive request.', 'error')
                return redirect(url_for('auth.role_select'))
        else:
            recv_type = 'staff'
        ...
        msg = Message(
            ...
            content=f'I have forgotten my password and am requesting a reset for Reg No: {student.reg_no}.'
        )
        db.session.add(msg)
        db.session.commit()
        flash('Password reset request sent successfully to your Staff coordinator.', 'success')

    elif role == 'staff':
        staff = User.query.filter_by(username=identifier, role='staff').first()
        if not staff:
            flash('Staff username not found.', 'error')
            return redirect(url_for('auth.role_select'))

        admin = User.query.filter_by(role='admin').first()
        if not admin:
            flash('System error: No admin found in the system.', 'error')
            return redirect(url_for('auth.role_select'))
        ...
        msg = Message(
            ...
            subject='URGENT: Staff Password Reset Request',
            content=f'I have forgotten my password and am requesting a reset for Staff username: {staff.username}.'
        )
        db.session.add(msg)
        db.session.commit()
        flash('Password reset request sent successfully to the Administrator.', 'success')
